#!/usr/bin/env python3
"""
Script to check Prolific assignments and session status in Docker database.

Features:
1. View all assigned participants (Prolific assignments)
...
def show_all_assignments(container_name='human-agent-postgres'):
    """Show all Prolific participant assignments"""
    print("="*80)
    print("📋 ALL PROLIFIC PARTICIPANT ASSIGNMENTS")
    print("="*80)
    ...
    query = """
        ...
        WHERE (p.prolific_pid IS NOT NULL 
               OR p.prolific_study_id IS NOT NULL 
               OR p.prolific_session_id IS NOT NULL)
    ...
    if not results:
        print("❌ No Prolific assignments found.")
        return
    ...
    for session_code, assignments in sorted(sessions.items()):
        ...
        for assignment in assignments:
            ...
            print(f"      Prolific PID: {assignment.get('prolific_pid') or 'N/A'}")
            print(f"      Study ID: {assignment.get('prolific_study_id') or 'N/A'}")
            print(f"      Prolific Session ID: {assignment.get('prolific_session_id') or 'N/A'}")
...
def show_unstarted_sessions(container_name='human-agent-postgres'):
    ...
    query = """
        SELECT 
            ...
            COUNT(CASE WHEN p.prolific_pid IS NOT NULL THEN 1 END) as assigned_count,
    ...
    for session in results:
        ...
        print(f"   Prolific Assigned: {session.get('assigned_count', 0)}")
        print()

def main():
    parser = argparse.ArgumentParser(
        description='Check Prolific assignments and session status in Docker database',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Show all Prolific participant assignments
  python check_prolific_assignments.py --assignments
  
  # Show all unstarted sessions
  python check_prolific_assignments.py --sessions
  
  # Show both
  python check_prolific_assignments.py --assignments --sessions
        """
    )
    
    parser.add_argument('--assignments', '-a', action='store_true', 
                       help='Show all Prolific participant assignments')
